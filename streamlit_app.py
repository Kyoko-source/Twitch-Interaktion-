...
# =========================
    ...
    let scene=null,state="menu",p=null,score=0,kills=0,seconds=0,saved=false,lastBoss=0,currentBoss=null,musicOn=true,audioCtx=null,moveKeys={};
    ...
    class SurvivorScene extends Phaser.Scene{
      ...
      resetRun(){if(this.enemies)this.enemies.getChildren().forEach(e=>this.destroyEnemyBar(e));p={hp:104,max:104,xp:0,need:34,level:1,speed:245,inv:0,up:{bolt:1},tim:{bolt:0,bomb:0,laser:0,drone:0,thunder:0,nova:3.8}};score=0;kills=0;seconds=0;saved=false;lastBoss=0;currentBoss=null;state="play";this.player.setPosition(WORLD_W/2,WORLD_H/2);this.player.body.enable=true;this.enemies.clear(true,true);this.bullets.clear(true,true);this.gems.clear(true,true);this.drops.clear(true,true);this.timeStart=this.time.now;hud()}
      ...
      wave(t){if(!this.nextWave||t>this.nextWave){this.nextWave=t+(seconds<30?880:seconds<70?700:Math.max(220,620-Math.floor(seconds)*2));const n=seconds<25?1:seconds<55?2:Math.min(15,3+Math.floor((seconds-55)/17));for(let i=0;i<n;i++)this.spawnEnemy()}const bm=Math.floor(seconds/150);if(bm>0&&bm!==lastBoss){lastBoss=bm;sfx("boss");this.spawnEnemy("boss")}}
      ...
      enemySpecial(e){if(this.time.now<e.nextSpecial)return;e.nextSpecial=this.time.now+(e.kind==="boss"?2100:2600);if(e.kind==="healer"){this.ring(e.x,e.y,130,0x7cffb2);this.enemies.getChildren().forEach(o=>{if(o!==e&&Phaser.Math.Distance.Between(o.x,o.y,e.x,e.y)<130){o.hp=Math.min(o.max,o.hp+o.max*.12);this.updateEnemyBar(o)}})}if(e.kind==="spitter"){this.ring(this.player.x,this.player.y,64,0x46f0ff);this.time.delayedCall(360,()=>{if(state==="play"&&Phaser.Math.Distance.Between(e.x,e.y,this.player.x,this.player.y)<240&&p.inv<=0)this.playerDamage(e,Phaser.Math.Angle.Between(e.x,e.y,this.player.x,this.player.y))})}if(e.kind==="boss"){this.ring(e.x,e.y,165,0xc77dff);if(Phaser.Math.Distance.Between(e.x,e.y,this.player.x,this.player.y)<165&&p.inv<=0)this.playerDamage(e,Phaser.Math.Angle.Between(e.x,e.y,this.player.x,this.player.y))}}
      playerDamage(e,a){const block=1-this.lv("shield")*.08;p.hp-=(e.kind==="boss"?28:e.kind==="tank"?16:e.kind==="runner"?11:10)*Math.max(.55,block);p.inv=52;sfx("hurt");this.cameras.main.shake(150,.006);this.ring(this.player.x,this.player.y,58,0xff4d6d);const push=230;this.player.setVelocity(Math.cos(a)*push,Math.sin(a)*push);e.setVelocity(-Math.cos(a)*push*.55,-Math.sin(a)*push*.55);if(p.hp<=0)this.over();hud()}
      ...
      bulletHit(b,e){this.hit(e,b.dmg);if(b.key==="bomb"){const r=this.evolved("bomb")?145:105;this.ring(b.x,b.y,r,0xffe66d);this.enemies.getChildren().forEach(o=>{if(Phaser.Math.Distance.Between(o.x,o.y,b.x,b.y)<r)this.hit(o,b.dmg*.58)});this.cameras.main.shake(90,.005)}if(b.pierce>0){b.pierce--;return}b.destroy()}
      popBullet(b){this.burst(b.x,b.y,0xffffff,5);b.destroy()}
      hit(e,d){if(!e.active)return;if(e.kind==="shield")d*=.62;e.hp-=d;this.updateEnemyBar(e);if(this.lv("frost")&&Math.random()<.55){e.slow=900+this.lv("frost")*160;e.setTint(0x9bf6ff)}if(Math.random()<.08)sfx("hit");if(e.hp<=0)this.kill(e)}
      kill(e){sfx("kill");kills++;score+=Math.floor(e.score+seconds*.8+p.level*2);this.blood(e.x,e.y,e.kind==="boss"||e.kind==="elite");this.burst(e.x,e.y,e.tintTopLeft||0xffffff,e.kind==="boss"?38:14);if(e.kind==="exploder"){this.ring(e.x,e.y,115,0xff4d6d);this.enemies.getChildren().forEach(o=>{if(o!==e&&Phaser.Math.Distance.Between(o.x,o.y,e.x,e.y)<115)this.hit(o,48)});if(Phaser.Math.Distance.Between(this.player.x,this.player.y,e.x,e.y)<105&&p.inv<=0)this.playerDamage(e,Phaser.Math.Angle.Between(e.x,e.y,this.player.x,this.player.y))}this.spawnGem(e.x,e.y,e.xp,e.kind==="boss"?1.7:1);if(Math.random()<.035||e.kind==="elite")this.spawnDrop(e.x,e.y,"heart");if(Math.random()<.022||e.kind==="boss")this.spawnDrop(e.x,e.y,"magnet");if(currentBoss===e)currentBoss=null;this.destroyEnemyBar(e);e.destroy();hud()}
    ...
    function hud(){if(!p)return;timeEl.textContent=time(seconds);scoreEl.textContent=score;levelEl.textContent=p.level;hpEl.textContent=Math.max(0,Math.ceil(p.hp));xpbar.style.width=Math.min(100,p.xp/p.need*100)+"%";hpbar.style.width=Math.min(100,p.hp/p.max*100)+"%";buildEl.textContent=Object.entries(p.up).filter(([,v])=>v>0).map(([k,v])=>(names[k]||k)+" "+v+(v>=(evo[k]||999)?"*":"")).join(" / ");if(currentBoss&&currentBoss.active){bossNameEl.textContent=currentBoss.kind==="boss"?"Mega Chicken":"Elite";bossbar.style.width=Math.max(0,Math.min(100,currentBoss.hp/currentBoss.max*100))+"%";bossInfo.textContent=Math.ceil(currentBoss.hp)+" / "+Math.ceil(currentBoss.max)+" HP"}else{bossNameEl.textContent="Keiner";bossbar.style.width="0%";bossInfo.textContent="Naechster Boss wartet"}}
