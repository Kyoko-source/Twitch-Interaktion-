...
# =========================
    ...
    let state="menu",p,enemies,bullets,gems,drops,parts,texts,obstacles,frame=0,seconds=0,score=0,kills=0,saved=false,lastBoss=0,shake=0,audioCtx=null,lastCluck=0,musicOn=true;
    const pool=[
    ...
    function makeObstacles(){obstacles=[{x:96,y:96,w:94,h:64,k:"house"},{x:780,y:88,w:112,h:72,k:"house"},{x:128,y:468,w:118,h:54,k:"stone"},{x:656,y:430,w:150,h:58,k:"stone"},{x:432,y:88,w:98,h:70,k:"house"},{x:312,y:132,w:70,h:42,k:"stone"},{x:492,y:420,w:82,h:44,k:"stone"}]}
    ...
    function reset(){p={x:W/2,y:H/2,r:17,hp:82,max:82,xp:0,need:34,level:1,speed:3.05,inv:0,up:{bolt:1},tim:{bolt:0,bomb:0,laser:0}};enemies=[];bullets=[];gems=[];drops=[];parts=[];texts=[];makeObstacles();frame=0;seconds=0;score=0;kills=0;saved=false;lastBoss=0;shake=0;lastCluck=0;hud()}
    ...
    function update(){let dx=0,dy=0;if(keys.arrowleft||keys.a)dx--;if(keys.arrowright||keys.d)dx++;if(keys.arrowup||keys.w)dy--;if(keys.arrowdown||keys.s)dy++;const l=Math.hypot(dx,dy)||1,ox=p.x,oy=p.y;p.x=clamp(p.x+dx/l*p.speed,p.r,W-p.r);obstacles.forEach(o=>pushOut(o,p));p.y=clamp(p.y+dy/l*p.speed,p.r,H-p.r);obstacles.forEach(o=>pushOut(o,p));p.inv=Math.max(0,p.inv-1);if(frame%60===0)p.hp=Math.min(p.max,p.hp+.16+lv("regen")*.55)}
