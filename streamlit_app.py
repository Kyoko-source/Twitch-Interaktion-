...
# =========================
    ...
    let scene=null,state="menu",p=null,score=0,kills=0,seconds=0,saved=false,lastBoss=0,musicOn=true,audioCtx=null,moveKeys={};
    ...
    function sfx(k,n=1){try{if(k==="start"){tone(420,.07,"triangle",.055);tone(640,.08,"triangle",.052,.08);tone(860,.1,"triangle",.05,.16)}if(k==="shoot"){for(let i=0;i<Math.min(n,7);i++)tone(760+Math.random()*300,.028,"square",.020,i*.01)}if(k==="bomb"){tone(145,.12,"sawtooth",.066);tone(72,.16,"sine",.038,.025);noise(.18,.048)}if(k==="laser"){tone(980,.05,"triangle",.058);tone(1480,.07,"sine",.044,.025);noise(.055,.018)}if(k==="drone"){tone(540,.045,"square",.034);tone(720,.035,"triangle",.022,.025)}if(k==="thunder"){tone(180,.045,"sawtooth",.068);tone(920,.07,"square",.046,.03);noise(.11,.042)}if(k==="nova"){tone(160,.13,"sine",.066);tone(420,.17,"triangle",.052,.04);noise(.14,.035,.02)}if(k==="pickup"){tone(740,.045,"sine",.040);tone(1040,.05,"triangle",.034,.035);tone(1320,.04,"sine",.024,.075)}if(k==="magnet"){tone(360,.08,"sine",.045);tone(900,.15,"triangle",.038,.04);noise(.13,.022)}if(k==="heart"){tone(520,.07,"sine",.044);tone(680,.09,"triangle",.035,.06)}if(k==="hit")tone(520+Math.random()*160,.035,"square",.030);if(k==="kill"){tone(720,.045,"triangle",.043);tone(480,.035,"square",.030,.025);if(Math.random()<.18){tone(360,.045,"square",.04,.03);tone(235,.07,"sawtooth",.033,.075)}}if(k==="hurt"){tone(180,.14,"sawtooth",.070);tone(95,.18,"square",.045,.06);noise(.12,.044)}if(k==="level"){tone(523,.08,"triangle",.055);tone(659,.08,"triangle",.058,.08);tone(784,.12,"triangle",.058,.16);tone(1046,.13,"sine",.044,.25)}if(k==="boss"){tone(90,.35,"sawtooth",.076);tone(140,.28,"square",.052,.08);noise(.35,.045)}}catch(e){}}
    class SurvivorScene extends Phaser.Scene{
      ...
      makeTextures(){
        ...
        g.fillStyle(0x51a65a,1).fillRect(0,0,128,128);g.lineStyle(1,0x276739,.45);for(let i=0;i<128;i+=32){g.lineBetween(i,0,i,128);g.lineBetween(0,i,128,i)}g.generateTexture("ground",128,128);g.clear();
        ...
        g.fillStyle(0x5d6474,1).fillRoundedRect(0,14,104,58,10);g.fillStyle(0x2b3142,1).fillTriangle(0,20,52,0,104,20);g.fillStyle(0x9bf6ff,1).fillRect(18,30,18,16).fillRect(52,28,18,14);g.generateTexture("lab",104,76);g.clear();
        g.fillStyle(0x7b8490,1).fillEllipse(62,34,124,62);g.fillStyle(0xffffff,.2).fillEllipse(44,22,38,18);g.generateTexture("rock",124,72);g.clear();
        g.fillStyle(0x46f0ff,.8);g.beginPath();g.moveTo(45,0);g.lineTo(86,70);g.lineTo(38,92);g.lineTo(0,60);g.closePath().fillPath();g.lineStyle(2,0xffffff,.35).strokePath();g.generateTexture("crystal",90,96);g.clear();g.destroy();
      }
      drawDecor(){for(let i=0;i<760;i++){const x=(i*337)%WORLD_W,y=(i*193)%WORLD_H,k=i%9;if(k===0)this.add.image(x,y,"spark").setTint(0x7cffb2).setAlpha(.35).setDepth(-5);else if(k===1)this.add.circle(x,y,3+(i%5),Phaser.Display.Color.HSLToColor((i%360)/360,.78,.68).color,.65).setDepth(-6);else this.add.rectangle(x,y,18+(i%20),3,0xffffff,.04).setDepth(-8)}}
      makeObstacles(){const data=[[360,260,"lab"],[900,520,"rock"],[1280,320,"lab"],[1960,280,"lab"],[2750,420,"crystal"],[3460,360,"lab"],[640,760,"rock"],[1680,980,"rock"],[2320,930,"lab"],[3220,1060,"crystal"],[1040,1180,"lab"],[2180,1260,"lab"],[520,1340,"rock"],[1430,1460,"rock"],[2740,1600,"lab"],[3650,1550,"lab"],[310,2050,"crystal"],[920,2210,"lab"],[1660,2360,"rock"],[2440,2260,"lab"],[3330,2380,"crystal"],[380,2780,"lab"],[1280,2820,"lab"],[2110,2760,"rock"],[3020,2820,"lab"]];data.forEach(o=>{const img=this.obstacles.create(o[0],o[1],o[2]).setDepth(5);img.refreshBody()})}
      resetRun(){if(this.enemies)this.enemies.getChildren().forEach(e=>this.destroyEnemyBar(e));p={hp:104,max:104,xp:0,need:34,level:1,speed:245,inv:0,up:{bolt:1},tim:{bolt:0,bomb:0,laser:0,drone:0,thunder:0,nova:3.8}};score=0;kills=0;seconds=0;saved=false;lastBoss=0;state="play";this.player.setPosition(WORLD_W/2,WORLD_H/2);this.player.body.enable=true;this.enemies.clear(true,true);this.bullets.clear(true,true);this.gems.clear(true,true);this.drops.clear(true,true);this.timeStart=this.time.now;hud()}
      lv(id){return p.up[id]||0}
      update(t,dt){if(state!=="play")return;seconds=(t-this.timeStart)/1000;this.move(dt);this.wave(t);this.fire(dt);this.mobs(dt);this.effects(t);if(t%1000<17){score+=Math.floor(2+seconds/40);p.hp=Math.min(p.max,p.hp+.18+this.lv("regen")*.6);hud()}}
      ...
      wave(t){if(!this.nextWave||t>this.nextWave){this.nextWave=t+(seconds<30?880:seconds<70?700:Math.max(220,620-Math.floor(seconds)*2));const n=seconds<25?1:seconds<55?2:Math.min(15,3+Math.floor((seconds-55)/17));for(let i=0;i<n;i++)this.spawnEnemy()}const bm=Math.floor(seconds/165);if(bm>0&&bm!==lastBoss){lastBoss=bm;sfx("boss");this.spawnEnemy("boss")}}
      spawnEnemy(kind){const r=Math.random();if(!kind)kind=seconds>130&&r<.10?"spitter":seconds>90&&r<.16?"tank":seconds>65&&r<.20?"elite":seconds>38&&r<.35?"runner":"grunt";const cam=this.cameras.main.worldView,s=Phaser.Math.Between(0,3),x=s===0?cam.left-100:s===1?cam.right+100:Phaser.Math.Between(cam.left,cam.right),y=s===2?cam.top-100:s===3?cam.bottom+100:Phaser.Math.Between(cam.top,cam.bottom),m=seconds/60,stats={grunt:[40+m*16,92+m*4,15,7,12,0xf8f7ff],runner:[30+m*13,150+m*5,12,8,16,0xffe66d],tank:[135+m*40,68+m*3,23,18,36,0xff9f1c],spitter:[70+m*22,86+m*3,16,14,28,0x46f0ff],elite:[285+m*78,96+m*3,28,42,110,0xff54a0],boss:[1220+m*340,72,45,145,500,0xc77dff]}[kind];const e=this.enemies.create(Phaser.Math.Clamp(x,30,WORLD_W-30),Phaser.Math.Clamp(y,30,WORLD_H-30),"chicken").setDepth(15).setTint(stats[5]).setScale(stats[2]/16);Object.assign(e,{kind,hp:stats[0],max:stats[0],speed:stats[1],base:stats[1],r:stats[2],xp:stats[3],score:stats[4],slow:0});e.body.setCircle(18);e.hpBg=this.add.rectangle(e.x,e.y-e.r-18,e.r*2.4,5,0x000000,.58).setDepth(24);e.hpBar=this.add.rectangle(e.x,e.y-e.r-18,e.r*2.4,5,0x7cffb2,.95).setDepth(25)}
      fire(dt){const cd=Math.max(.44,1-this.lv("cooldown")*.07),might=1+this.lv("might")*.16;p.tim.bolt-=dt/1000;if(p.tim.bolt<=0&&this.enemies.countActive()){const target=this.nearest(),amt=this.lv("bolt");sfx("shoot",amt);for(let i=0;i<amt;i++)this.shootAt(this.player.x,this.player.y,target,"bolt",(i-(amt-1)/2)*.15,(14+this.lv("frost")*2)*might);p.tim.bolt=Math.max(.15,(.50-this.lv("bolt")*.03)*cd)}if(this.lv("bomb")){p.tim.bomb-=dt/1000;if(p.tim.bomb<=0&&this.enemies.countActive()){sfx("bomb");this.shootAt(this.player.x,this.player.y,this.nearest(),"bomb",0,(44+this.lv("bomb")*19)*might);p.tim.bomb=Math.max(.38,(1.30-this.lv("bomb")*.12)*cd)}}if(this.lv("laser")){p.tim.laser-=dt/1000;if(p.tim.laser<=0&&this.enemies.countActive()){const e=this.enemies.getChildren().sort((a,b)=>b.hp-a.hp)[0];sfx("laser");this.laser(e,(62+this.lv("laser")*31)*might);p.tim.laser=Math.max(.78,(2.16-this.lv("laser")*.18)*cd)}}if(this.lv("drone")){p.tim.drone-=dt/1000;if(p.tim.drone<=0&&this.enemies.countActive()){sfx("drone");for(let i=0;i<this.lv("drone");i++){const a=tRadians(this.time.now/280+i*360/this.lv("drone"));this.shootAt(this.player.x+Math.cos(a)*48,this.player.y+Math.sin(a)*48,this.nearest(),"bolt",0,(12+this.lv("drone")*4)*might,0xc77dff)}p.tim.drone=Math.max(.20,(.86-this.lv("drone")*.08)*cd)}}if(this.lv("thunder")){p.tim.thunder-=dt/1000;if(p.tim.thunder<=0&&this.enemies.countActive()){sfx("thunder");let ox=this.player.x,oy=this.player.y;this.enemies.getChildren().sort((a,b)=>Phaser.Math.Distance.Between(a.x,a.y,this.player.x,this.player.y)-Phaser.Math.Distance.Between(b.x,b.y,this.player.x,this.player.y)).slice(0,2+this.lv("thunder")).forEach(e=>{this.line(ox,oy,e.x,e.y,0xf8f7ff);this.hit(e,(24+this.lv("thunder")*9)*might);ox=e.x;oy=e.y});p.tim.thunder=Math.max(.54,(1.68-this.lv("thunder")*.13)*cd)}}if(this.lv("nova")){p.tim.nova-=dt/1000;if(p.tim.nova<=0){const r=120+this.lv("nova")*28;sfx("nova");this.ring(this.player.x,this.player.y,r,0xff54a0);this.enemies.getChildren().forEach(e=>{if(Phaser.Math.Distance.Between(e.x,e.y,this.player.x,this.player.y)<r)this.hit(e,(28+this.lv("nova")*15)*might)});this.cameras.main.shake(110,.004);p.tim.nova=Math.max(.92,(2.60-this.lv("nova")*.22)*cd)}}}
      shootAt(x,y,target,key,spread,dmg,tint){if(!target)return;const a=Phaser.Math.Angle.Between(x,y,target.x,target.y)+spread,b=this.bullets.create(x,y,key==="bomb"?"bomb":"bolt").setDepth(18).setTint(tint||0xffffff);b.setVelocity(Math.cos(a)*(key==="bomb"?330:560),Math.sin(a)*(key==="bomb"?330:560));Object.assign(b,{dmg,key,life:this.time.now+(key==="bomb"?1600:1200)})}
      ...
      mobs(dt){this.enemies.getChildren().forEach(e=>{e.slow=Math.max(0,e.slow-dt);const a=Phaser.Math.Angle.Between(e.x,e.y,this.player.x,this.player.y),speed=e.base*(e.slow>0?.55:1);e.setVelocity(Math.cos(a)*speed,Math.sin(a)*speed);e.rotation=Math.sin(this.time.now*.008+e.x)*.12;this.updateEnemyBar(e);if(this.lv("aura")&&Phaser.Math.Distance.Between(e.x,e.y,this.player.x,this.player.y)<40+this.lv("aura")*15)this.hit(e,.22*this.lv("aura")*(1+this.lv("might")*.16));if(Phaser.Math.Distance.Between(e.x,e.y,this.player.x,this.player.y)<e.r+20&&p.inv<=0){const block=1-this.lv("shield")*.08;p.hp-=(e.kind==="boss"?28:e.kind==="tank"?16:e.kind==="runner"?11:10)*Math.max(.55,block);p.inv=430;sfx("hurt");this.cameras.main.shake(140,.006);if(p.hp<=0)this.over();hud()}})}
      effects(t){this.bullets.getChildren().forEach(b=>{if(t>b.life)b.destroy()});for(let i=0;i<this.lv("orbit");i++){if(!this.orbs)this.orbs=[];if(!this.orbs[i])this.orbs[i]=this.add.image(this.player.x,this.player.y,"gem").setDepth(22).setScale(.42);const a=tRadians(t/18+i*360/this.lv("orbit")),x=this.player.x+Math.cos(a)*(58+this.lv("orbit")*5),y=this.player.y+Math.sin(a)*(58+this.lv("orbit")*5);this.orbs[i].setPosition(x,y);this.enemies.getChildren().forEach(e=>{if(Phaser.Math.Distance.Between(e.x,e.y,x,y)<e.r+16)this.hit(e,.52*this.lv("orbit")*(1+this.lv("might")*.16))})}if(this.orbs)for(let i=this.lv("orbit");i<this.orbs.length;i++){this.orbs[i].destroy();this.orbs.splice(i,1)}for(let i=0;i<this.lv("drone");i++){if(!this.drones)this.drones=[];if(!this.drones[i])this.drones[i]=this.add.image(this.player.x,this.player.y,"coreGlow").setDepth(21).setScale(.22).setTint(0xc77dff);const a=tRadians(t/28+i*360/Math.max(1,this.lv("drone")));this.drones[i].setPosition(this.player.x+Math.cos(a)*48,this.player.y+Math.sin(a)*48)}}
      bulletHit(b,e){this.hit(e,b.dmg);if(b.key==="bomb"){this.ring(b.x,b.y,105,0xffe66d);this.enemies.getChildren().forEach(o=>{if(Phaser.Math.Distance.Between(o.x,o.y,b.x,b.y)<105)this.hit(o,b.dmg*.58)});this.cameras.main.shake(90,.005)}b.destroy()}
      popBullet(b){this.burst(b.x,b.y,0xffffff,5);b.destroy()}
      hit(e,d){if(!e.active)return;e.hp-=d;this.updateEnemyBar(e);if(this.lv("frost")&&Math.random()<.55){e.slow=900+this.lv("frost")*160;e.setTint(0x9bf6ff)}if(Math.random()<.08)sfx("hit");if(e.hp<=0)this.kill(e)}
      kill(e){sfx("kill");kills++;score+=Math.floor(e.score+seconds*.8+p.level*2);this.blood(e.x,e.y,e.kind==="boss"||e.kind==="elite");this.burst(e.x,e.y,e.tintTopLeft||0xffffff,e.kind==="boss"?38:14);this.spawnGem(e.x,e.y,e.xp,e.kind==="boss"?1.35:1);if(Math.random()<.035||e.kind==="elite")this.spawnDrop(e.x,e.y,"heart");if(Math.random()<.022||e.kind==="boss")this.spawnDrop(e.x,e.y,"magnet");this.destroyEnemyBar(e);e.destroy();hud()}
      updateEnemyBar(e){if(!e.hpBg||!e.hpBar)return;const w=e.r*2.4,pct=Phaser.Math.Clamp(e.hp/e.max,0,1),y=e.y-e.r-18;e.hpBg.setPosition(e.x,y).setSize(w,5);e.hpBar.setPosition(e.x-w*(1-pct)/2,y).setSize(w*pct,5).setFillStyle(e.slow>0?0x9bf6ff:0x7cffb2,.95)}
      ...
      pickDrop(d){if(d.k==="heart"){sfx("heart");p.hp=Math.min(p.max,p.hp+34)}else{sfx("magnet");this.gems.getChildren().forEach(g=>{this.gainXp(g.v);g.destroy()});this.ring(this.player.x,this.player.y,190,0x46f0ff)}d.destroy();hud()}
      gainXp(v){p.xp+=v;while(p.xp>=p.need){p.xp-=p.need;p.level++;p.need=Math.floor(p.need*1.24+18);p.hp=Math.min(p.max,p.hp+8);score+=p.level*25;sfx("level");this.choose();break}}
      ...
      applyUpgrade(id){p.up[id]=this.lv(id)+1;if(id==="speed")p.speed+=34;if(id==="shield"){p.max+=16;p.hp=Math.min(p.max,p.hp+28)}if(id==="regen"){p.max+=10;p.hp=Math.min(p.max,p.hp+26)}this.ring(this.player.x,this.player.y,150,0x7cffb2);overlay.style.display="none";upgradesEl.innerHTML="";state="play";this.scene.resume();hud()}
      over(){stopMusic();state="over";this.player.body.enable=false;overlay.style.display="flex";upgradesEl.innerHTML="";kicker.textContent="Run beendet";title.textContent="Game Over";text.textContent="Zeit: "+time(seconds)+" | Level: "+p.level+" | Kills: "+kills+" | Score: "+score;startBtn.textContent="Nochmal spielen";startBtn.style.display="inline-flex";saveBtn.style.display=score>0&&!saved?"inline-flex":"none";restartBtn.style.display="none";musicBtn.style.display="inline-flex";startBtn.onclick=start}
      ...
      laser(e,d){this.line(this.player.x,this.player.y,e.x,e.y,0x46f0ff);this.hit(e,d);this.cameras.main.shake(70,.003)}
    ...
    if(window.Phaser){new Phaser.Game({type:Phaser.AUTO,parent:"game",backgroundColor:"#07101f",width:1000,height:620,scale:{mode:Phaser.Scale.FIT,autoCenter:Phaser.Scale.CENTER_BOTH},physics:{default:"arcade",arcade:{debug:false}},scene:SurvivorScene})}else{text.textContent="Phaser konnte nicht geladen werden. Internet/CDN pruefen.";startBtn.style.display="none"}
    ...
    document.addEventListener("keydown",e=>{const k=e.key.toLowerCase();moveKeys[k]=true;if(["arrowup","arrowdown","arrowleft","arrowright"," "].includes(k))e.preventDefault()});document.addEventListener("keyup",e=>{moveKeys[e.key.toLowerCase()]=false});
    startBtn.onclick=start;saveBtn.onclick=saveScore;restartBtn.onclick=start;musicBtn.onclick=()=>{musicOn=!musicOn;musicBtn.textContent=musicOn?"Musik: An":"Musik: Aus";if(!musicOn)stopMusic();else if(state==="play")startMusic()};saveBtn.style.display="none";restartBtn.style.display="none";renderScores();
    </script></body></html>
