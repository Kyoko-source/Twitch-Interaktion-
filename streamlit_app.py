...
# =========================
    ...
    <div class="shell"><div class="layout"><div class="stage">
        ...
        <div id="overlay" class="overlay"><div class="panel">
            ...
            <div class="actions"><button id="start">Run starten</button><button id="save" class="secondary">Score speichern</button><button id="restart" class="secondary">Neu starten</button><button id="musicToggle" class="secondary">Musik: An</button></div>
        </div></div>
    ...
    const startBtn=document.getElementById("start"),saveBtn=document.getElementById("save"),restartBtn=document.getElementById("restart"),musicBtn=document.getElementById("musicToggle"),musicFile=document.getElementById("musicFile");
    ...
    const W=canvas.width,H=canvas.height,WORLD_W=4200,WORLD_H=3100,keys={};
    let state="menu",p,enemies,bullets,gems,drops,parts,texts,obstacles,decor,cam={x:0,y:0},frame=0,seconds=0,score=0,kills=0,saved=false,lastBoss=0,shake=0,audioCtx=null,lastCluck=0,musicOn=true;
    const pool=[
      ["bolt","Brain Bolt","+1 Projektil, schnelleres Auto-Aim",8],["orbit","Synapsen-Orbit","Kreisende Gehirnzellen zerlegen Nahkaempfer",7],["aura","Neural Aura","Schadenszone um dich herum",7],["bomb","Egg Bomb","Explosive Eier mit Flaechenschaden",6],["laser","Focus Laser","Periodischer Laser auf den staerksten Gegner",6],["drone","Synapse Drone","Begleiter feuern zusaetzliche Plasmashots",5],["thunder","Thought Thunder","Kettenblitze springen zwischen Gegnern",5],["nova","Panic Nova","Regelmaessige Schockwelle raeumt Platz",5],["frost","Memory Freeze","Treffer verlangsamen Gegner",5],["shield","Mind Shield","Mehr Max-HP und weniger Kontaktschaden",5],["speed","Chicken Boots","+12% Bewegungstempo",5],["magnet","Big Brain Magnet","Mehr Pickup-Reichweite",6],["might","Gehirnzellen-Power","+18% Gesamtschaden",8],["regen","Warm Nest","Regeneration und +10 Max-HP",6],["cooldown","Turbo Synapse","Alle Waffen laden schneller",7]
    ];
    ...
    function startMusic(){if(!musicOn||!musicFile||!musicFile.getAttribute("src"))return;musicFile.volume=.28;musicFile.play().catch(()=>{})}
    ...
    function sfx(k,n=1){try{if(k==="start"){tone(420,.07);tone(640,.08,"triangle",.04,.08);tone(860,.1,"triangle",.04,.16)}if(k==="shoot"){for(let i=0;i<Math.min(n,7);i++)tone(760+Math.random()*300,.026,"square",.011,i*.01)}if(k==="bomb"){tone(145,.11,"sawtooth",.04);noise(.16,.03)}if(k==="laser"){tone(980,.045,"triangle",.035);tone(1480,.06,"sine",.025,.025)}if(k==="drone")tone(540,.04,"square",.018);if(k==="thunder"){tone(180,.04,"sawtooth",.045);tone(920,.06,"square",.024,.03);noise(.09,.026)}if(k==="nova"){tone(160,.12,"sine",.042);tone(420,.16,"triangle",.03,.04)}if(k==="pickup"){tone(740,.045,"sine",.025);tone(1040,.05,"triangle",.018,.035)}if(k==="hit")tone(520+Math.random()*160,.035,"square",.018);if(k==="kill"){tone(720,.045,"triangle",.026);if(Math.random()<.16){tone(360,.045,"square",.03,.03);tone(235,.07,"sawtooth",.025,.075)}}if(k==="hurt"){tone(180,.14,"sawtooth",.055);tone(95,.18,"square",.035,.06);noise(.12,.032)}if(k==="level"){tone(523,.08);tone(659,.08,"triangle",.045,.08);tone(784,.12,"triangle",.045,.16);tone(1046,.13,"sine",.035,.25)}if(k==="boss"){tone(90,.35,"sawtooth",.06);tone(140,.28,"square",.04,.08);noise(.35,.03)}}catch(e){}}
    function burst(x,y,c,n=12,power=4){for(let j=0;j<n;j++)parts.push({x,y,vx:(Math.random()-.5)*power,vy:(Math.random()-.5)*power,life:18+Math.random()*22,c,r:2+Math.random()*3})}
    function bloodBurst(x,y,big=false){const n=big?34:16,p=big?8:5;for(let j=0;j<n;j++){const a=Math.random()*Math.PI*2,s=(.6+Math.random())*p;parts.push({x:x+(Math.random()-.5)*10,y:y+(Math.random()-.5)*10,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life:16+Math.random()*26,c:Math.random()<.25?"#7a1022":"#c9184a",r:1.8+Math.random()*(big?4.2:2.8),blood:true})}}
    function makeObstacles(){obstacles=[{x:360,y:260,w:130,h:80,k:"lab"},{x:900,y:520,w:130,h:62,k:"stone"},{x:1280,y:320,w:148,h:86,k:"house"},{x:1960,y:280,w:152,h:86,k:"lab"},{x:2750,y:420,w:164,h:72,k:"crystal"},{x:3460,y:360,w:170,h:92,k:"house"},{x:640,y:760,w:150,h:64,k:"stone"},{x:1680,y:980,w:190,h:64,k:"stone"},{x:2320,y:930,w:130,h:130,k:"tower"},{x:3220,y:1060,w:180,h:76,k:"crystal"},{x:1040,y:1180,w:132,h:86,k:"house"},{x:2180,y:1260,w:168,h:92,k:"lab"},{x:520,y:1340,w:120,h:58,k:"stone"},{x:1430,y:1460,w:170,h:66,k:"stone"},{x:2740,y:1600,w:160,h:92,k:"house"},{x:3650,y:1550,w:170,h:78,k:"tower"},{x:310,y:2050,w:160,h:76,k:"crystal"},{x:920,y:2210,w:170,h:88,k:"lab"},{x:1660,y:2360,w:190,h:74,k:"stone"},{x:2440,y:2260,w:165,h:86,k:"house"},{x:3330,y:2380,w:185,h:76,k:"crystal"},{x:380,y:2780,w:150,h:78,k:"house"},{x:1280,y:2820,w:160,h:70,k:"tower"},{x:2110,y:2760,w:190,h:76,k:"stone"},{x:3020,y:2820,w:170,h:84,k:"lab"}];decor=[];for(let i=0;i<520;i++)decor.push({x:(i*337)%WORLD_W,y:(i*193)%WORLD_H,s:3+(i%9),k:i%11===0?"spark":i%7===0?"flower":i%5===0?"tile":"grass",h:(i*47)%360})}
    function circleRect(o,x,y,r){const cx=clamp(x,o.x,o.x+o.w),cy=clamp(y,o.y,o.y+o.h);return Math.hypot(x-cx,y-cy)<r}
    function pushOut(o,obj){const inside=obj.x>o.x&&obj.x<o.x+o.w&&obj.y>o.y&&obj.y<o.y+o.h;if(inside){const l=obj.x-o.x,r=o.x+o.w-obj.x,t=obj.y-o.y,b=o.y+o.h-obj.y,m=Math.min(l,r,t,b);if(m===l)obj.x=o.x-obj.r-1;else if(m===r)obj.x=o.x+o.w+obj.r+1;else if(m===t)obj.y=o.y-obj.r-1;else obj.y=o.y+o.h+obj.r+1;return true}const cx=clamp(obj.x,o.x,o.x+o.w),cy=clamp(obj.y,o.y,o.y+o.h),dx=obj.x-cx,dy=obj.y-cy,d=Math.hypot(dx,dy)||1;if(Math.hypot(dx,dy)<obj.r){obj.x=cx+dx/d*(obj.r+1);obj.y=cy+dy/d*(obj.r+1);return true}return false}
    function reset(){p={x:WORLD_W/2,y:WORLD_H/2,r:18,hp:96,max:96,xp:0,need:34,level:1,speed:3.15,inv:0,trail:[],up:{bolt:1},tim:{bolt:0,bomb:0,laser:0,drone:0,thunder:0,nova:220}};cam={x:p.x-W/2,y:p.y-H/2};enemies=[];bullets=[];gems=[];drops=[];parts=[];texts=[];makeObstacles();frame=0;seconds=0;score=0;kills=0;saved=false;lastBoss=0;shake=0;lastCluck=0;hud()}
    function start(){audio();sfx("start");startMusic();reset();state="play";overlay.style.display="none"}
    function over(){stopMusic();state="over";overlay.style.display="flex";upgradesEl.innerHTML="";kicker.textContent="Run beendet";title.textContent="Game Over";text.textContent="Zeit: "+time(seconds)+" | Level: "+p.level+" | Kills: "+kills+" | Score: "+score;startBtn.textContent="Nochmal spielen";startBtn.style.display="inline-flex";saveBtn.style.display=score>0&&!saved?"inline-flex":"none";restartBtn.style.display="none";musicBtn.style.display="inline-flex";startBtn.onclick=start}
    ...
    function apply(id){p.up[id]=lv(id)+1;if(id==="speed")p.speed+=.42;if(id==="shield"){p.max+=16;p.hp=Math.min(p.max,p.hp+28)}if(id==="regen"){p.max+=10;p.hp=Math.min(p.max,p.hp+26)}sfx("level");burst(p.x,p.y,"#7cffb2",34,8);state="play";overlay.style.display="none";upgradesEl.innerHTML="";hud()}
    function spawn(kind="grunt"){const s=Math.floor(Math.random()*4),left=cam.x-95,right=cam.x+W+95,top=cam.y-95,bottom=cam.y+H+95,pos=[{x:left,y:cam.y+Math.random()*H},{x:right,y:cam.y+Math.random()*H},{x:cam.x+Math.random()*W,y:top},{x:cam.x+Math.random()*W,y:bottom}][s],m=seconds/60;pos.x=clamp(pos.x,24,WORLD_W-24);pos.y=clamp(pos.y,24,WORLD_H-24);
      const t={grunt:[34+m*16,1.18+m*.065,15,7,12,"#f8f7ff"],runner:[26+m*13,1.92+m*.075,12,8,16,"#ffe66d"],tank:[126+m*40,.84+m*.04,22,18,36,"#ff9f1c"],spitter:[64+m*21,1.02+m*.04,16,14,28,"#46f0ff"],elite:[270+m*76,1.14+m*.04,27,42,110,"#ff54a0"],boss:[1180+m*330,.92,44,145,500,"#c77dff"]}[kind];
      enemies.push({...pos,kind,hp:t[0],max:t[0],speed:t[1],base:t[1],r:t[2],xp:t[3],score:t[4],color:t[5],hit:0,slow:0})}
    function wave(){const n=seconds<25?1:seconds<55?2:Math.min(15,3+Math.floor((seconds-55)/17));for(let i=0;i<n;i++){const r=Math.random(); if(seconds>130&&r<.10)spawn("spitter"); else if(seconds>90&&r<.16)spawn("tank"); else if(seconds>38&&r<.34)spawn("runner"); else spawn("grunt")} if(seconds>65&&frame%330===0)spawn("elite");const bm=Math.floor(seconds/165);if(bm>0&&bm!==lastBoss){lastBoss=bm;sfx("boss");spawn("boss")}}
    ...
    function fire(){const cd=Math.max(.44,1-lv("cooldown")*.07),might=1+lv("might")*.16;p.tim.bolt--;if(p.tim.bolt<=0&&enemies.length){const t=nearest(),amt=lv("bolt");sfx("shoot",amt);for(let i=0;i<amt;i++){const a=Math.atan2(t.y-p.y,t.x-p.x)+(i-(amt-1)/2)*.15;bullets.push({x:p.x,y:p.y,vx:Math.cos(a)*7.5,vy:Math.sin(a)*7.5,r:5,d:(14+lv("frost")*2)*might,life:82,k:"bolt",c:lv("frost")?"#9bf6ff":"#7cffb2"})}p.tim.bolt=Math.max(15,(50-lv("bolt")*3)*cd)}
      if(lv("bomb")>0){p.tim.bomb--;if(p.tim.bomb<=0&&enemies.length){const t=nearest(),a=Math.atan2(t.y-p.y,t.x-p.x);sfx("bomb");bullets.push({x:p.x,y:p.y,vx:Math.cos(a)*4.6,vy:Math.sin(a)*4.6,r:10,d:(44+lv("bomb")*19)*might,life:95,k:"bomb",c:"#ffe66d"});p.tim.bomb=Math.max(38,(130-lv("bomb")*12)*cd)}}
      if(lv("laser")>0){p.tim.laser--;if(p.tim.laser<=0&&enemies.length){const t=enemies.slice().sort((a,b)=>b.hp-a.hp)[0];sfx("laser");hit(t,(62+lv("laser")*31)*might,true);parts.push({x1:p.x,y1:p.y,x2:t.x,y2:t.y,life:14,laser:true,c:"#46f0ff"});p.tim.laser=Math.max(78,(216-lv("laser")*18)*cd)}}
      if(lv("drone")>0){p.tim.drone--;if(p.tim.drone<=0&&enemies.length){sfx("drone");for(let i=0;i<lv("drone");i++){const a=frame/28+i*Math.PI*2/lv("drone"),sx=p.x+Math.cos(a)*42,sy=p.y+Math.sin(a)*42,t=nearest(),aa=Math.atan2(t.y-sy,t.x-sx);bullets.push({x:sx,y:sy,vx:Math.cos(aa)*6.3,vy:Math.sin(aa)*6.3,r:4,d:(12+lv("drone")*4)*might,life:72,k:"drone",c:"#c77dff"})}p.tim.drone=Math.max(20,(86-lv("drone")*8)*cd)}}
      if(lv("thunder")>0){p.tim.thunder--;if(p.tim.thunder<=0&&enemies.length){sfx("thunder");let chain=enemies.slice().sort((a,b)=>Math.hypot(a.x-p.x,a.y-p.y)-Math.hypot(b.x-p.x,b.y-p.y)).slice(0,2+lv("thunder"));let ox=p.x,oy=p.y;chain.forEach(e=>{hit(e,(24+lv("thunder")*9)*might,true);parts.push({x1:ox,y1:oy,x2:e.x,y2:e.y,life:9,laser:true,c:"#f8f7ff"});ox=e.x;oy=e.y});p.tim.thunder=Math.max(54,(168-lv("thunder")*13)*cd)}}
      if(lv("nova")>0){p.tim.nova--;if(p.tim.nova<=0){const r=120+lv("nova")*28;sfx("nova");parts.push({x:p.x,y:p.y,life:24,ring:true,r:18,max:r,c:"#ff54a0"});enemies.slice().forEach(e=>{if(Math.hypot(e.x-p.x,e.y-p.y)<r)hit(e,(28+lv("nova")*15)*might,true)});shake=Math.min(9,shake+3);p.tim.nova=Math.max(92,(260-lv("nova")*22)*cd)}}}
    function hit(e,d,boom=false){if(!e)return;e.hp-=d;e.hit=5;if(lv("frost")&&Math.random()<.55)e.slow=70+lv("frost")*18;if(Math.random()<.11)sfx("hit");if(Math.random()<.18)texts.push({x:e.x,y:e.y-e.r,v:Math.floor(d),life:26});if(e.hp<=0)kill(e);if(boom)shake=Math.min(8,shake+2)}
    function kill(e){const i=enemies.indexOf(e);if(i>=0)enemies.splice(i,1);sfx("kill");kills++;score+=Math.floor(e.score+seconds*.8+p.level*2);gems.push({x:e.x,y:e.y,v:e.xp,r:e.kind==="boss"?10:6});if(Math.random()<.032||e.kind==="elite")drops.push({x:e.x,y:e.y,k:"heart",r:8});if(Math.random()<.02||e.kind==="boss")drops.push({x:e.x,y:e.y,k:"magnet",r:8});bloodBurst(e.x,e.y,e.kind==="boss"||e.kind==="elite");burst(e.x,e.y,e.color,e.kind==="boss"?42:14,e.kind==="boss"?9:5)}
    function xp(v){p.xp+=v;while(p.xp>=p.need){p.xp-=p.need;p.level++;p.need=Math.floor(p.need*1.24+18);p.hp=Math.min(p.max,p.hp+7);score+=p.level*25;sfx("level");choose();break}}
    function update(){let dx=0,dy=0;if(keys.arrowleft||keys.a)dx--;if(keys.arrowright||keys.d)dx++;if(keys.arrowup||keys.w)dy--;if(keys.arrowdown||keys.s)dy++;const l=Math.hypot(dx,dy)||1;p.x=clamp(p.x+dx/l*p.speed,p.r,WORLD_W-p.r);obstacles.forEach(o=>pushOut(o,p));p.y=clamp(p.y+dy/l*p.speed,p.r,WORLD_H-p.r);obstacles.forEach(o=>pushOut(o,p));cam.x=clamp(p.x-W/2,0,WORLD_W-W);cam.y=clamp(p.y-H/2,0,WORLD_H-H);p.inv=Math.max(0,p.inv-1);p.trail.unshift({x:p.x,y:p.y,life:24});p.trail=p.trail.filter(t=>--t.life>0).slice(0,24);if(frame%60===0)p.hp=Math.min(p.max,p.hp+.18+lv("regen")*.58)}
    function mobs(){const aura=lv("aura"),range=40+aura*15,orbit=lv("orbit");enemies.slice().forEach(e=>{const a=Math.atan2(p.y-e.y,p.x-e.x);e.slow=Math.max(0,e.slow-1);e.speed=(e.base||e.speed)*(e.slow>0?.55:1);e.x+=Math.cos(a)*e.speed;e.y+=Math.sin(a)*e.speed;obstacles.forEach(o=>{if(pushOut(o,e)){e.x+=(Math.random()-.5)*1.8;e.y+=(Math.random()-.5)*1.8}});e.hit=Math.max(0,e.hit-1);if(aura&&Math.hypot(e.x-p.x,e.y-p.y)<range)hit(e,.22*aura*(1+lv("might")*.16));if(orbit){for(let i=0;i<orbit;i++){const oa=frame/27+i*Math.PI*2/orbit,ox=p.x+Math.cos(oa)*(50+orbit*5),oy=p.y+Math.sin(oa)*(50+orbit*5);if(Math.hypot(e.x-ox,e.y-oy)<e.r+10)hit(e,.52*orbit*(1+lv("might")*.16))}} if(Math.hypot(e.x-p.x,e.y-p.y)<e.r+p.r&&p.inv<=0){const block=1-lv("shield")*.08;p.hp-=(e.kind==="boss"?28:e.kind==="tank"?16:e.kind==="runner"?11:10)*Math.max(.55,block);sfx("hurt");p.inv=28;shake=8;if(p.hp<=0)over()}})}
    ...
    function pickups(){const mag=82+lv("magnet")*40;gems.slice().forEach(g=>{const d=Math.hypot(g.x-p.x,g.y-p.y);if(d<mag){g.x+=(p.x-g.x)*.12;g.y+=(p.y-g.y)*.12}if(d<p.r+g.r+4){sfx("pickup");xp(g.v);gems.splice(gems.indexOf(g),1)}});drops.slice().forEach(d=>{const q=Math.hypot(d.x-p.x,d.y-p.y);if(q<mag){d.x+=(p.x-d.x)*.09;d.y+=(p.y-d.y)*.09}if(q<p.r+d.r+4){sfx("pickup");if(d.k==="heart")p.hp=Math.min(p.max,p.hp+34);if(d.k==="magnet"){gems.forEach(g=>xp(g.v));gems=[];parts.push({x:p.x,y:p.y,life:28,ring:true,r:20,max:180,c:"#46f0ff"})}drops.splice(drops.indexOf(d),1)}})}
    function hud(){timeEl.textContent=time(seconds);scoreEl.textContent=score;levelEl.textContent=p.level;hpEl.textContent=Math.max(0,Math.ceil(p.hp));xpbar.style.width=Math.min(100,p.xp/p.need*100)+"%";hpbar.style.width=Math.min(100,p.hp/p.max*100)+"%";buildEl.textContent=Object.entries(p.up).filter(([,v])=>v>0).map(([k,v])=>(names[k]||k)+" "+v).join(" · ")}
    ...
    function shots(){bullets.slice().forEach(b=>{b.x+=b.vx;b.y+=b.vy;b.life--;if(obstacles.some(o=>circleRect(o,b.x,b.y,b.r))){b.life=0;burst(b.x,b.y,"#ffffff",4,2);return}enemies.slice().forEach(e=>{if(Math.hypot(e.x-b.x,e.y-b.y)<e.r+b.r){hit(e,b.d);if(b.k==="bomb"){parts.push({x:b.x,y:b.y,life:22,ring:true,r:18,max:96,c:"#ffe66d"});enemies.slice().forEach(o=>{if(Math.hypot(o.x-b.x,o.y-b.y)<96)hit(o,b.d*.58)});shake=Math.min(9,shake+4)}b.life=0}})});bullets=bullets.filter(b=>b.life>0&&b.x>-60&&b.x<WORLD_W+60&&b.y>-60&&b.y<WORLD_H+60)}
    function drawObstacle(o){ctx.save();ctx.shadowColor="rgba(0,0,0,.24)";ctx.shadowBlur=14;ctx.shadowOffsetY=7;if(o.k==="house"||o.k==="lab"){ctx.fillStyle=o.k==="lab"?"#3f5f72":"#8b5a32";ctx.fillRect(o.x+8,o.y+24,o.w-16,o.h-20);ctx.fillStyle=o.k==="lab"?"#1b2c3b":"#5b2f27";ctx.beginPath();ctx.moveTo(o.x,o.y+30);ctx.lineTo(o.x+o.w/2,o.y);ctx.lineTo(o.x+o.w,o.y+30);ctx.closePath();ctx.fill();ctx.fillStyle="#9bf6ff";ctx.fillRect(o.x+o.w*.16,o.y+38,20,16);ctx.fillRect(o.x+o.w*.44,o.y+36,18,14);ctx.fillStyle="#2b1b18";ctx.fillRect(o.x+o.w*.68,o.y+42,20,32)}else if(o.k==="tower"){ctx.fillStyle="#5d6474";ctx.fillRect(o.x+o.w*.22,o.y+10,o.w*.56,o.h-10);ctx.fillStyle="#2b3142";ctx.fillRect(o.x+8,o.y,o.w-16,22);ctx.fillStyle="#ff54a0";ctx.beginPath();ctx.arc(o.x+o.w*.5,o.y+10,7,0,Math.PI*2);ctx.fill()}else if(o.k==="crystal"){ctx.fillStyle="#46f0ff";ctx.globalAlpha=.82;ctx.beginPath();ctx.moveTo(o.x+o.w*.5,o.y);ctx.lineTo(o.x+o.w,o.y+o.h*.72);ctx.lineTo(o.x+o.w*.42,o.y+o.h);ctx.lineTo(o.x,o.y+o.h*.62);ctx.closePath();ctx.fill();ctx.globalAlpha=1;ctx.strokeStyle="rgba(255,255,255,.35)";ctx.stroke()}else{ctx.fillStyle="#7b8490";ctx.beginPath();ctx.ellipse(o.x+o.w*.5,o.y+o.h*.55,o.w*.5,o.h*.45,0,0,Math.PI*2);ctx.fill();ctx.fillStyle="rgba(255,255,255,.18)";ctx.beginPath();ctx.ellipse(o.x+o.w*.34,o.y+o.h*.35,o.w*.18,o.h*.12,-.4,0,Math.PI*2);ctx.fill()}ctx.restore()}
    function drawChicken(e){ctx.save();ctx.translate(e.x,e.y);const bob=Math.sin(frame/5+e.x*.01)*2,scale=e.r/16;ctx.scale(scale,scale);ctx.translate(0,bob);ctx.fillStyle="rgba(0,0,0,.24)";ctx.beginPath();ctx.ellipse(4,17,18,5,0,0,Math.PI*2);ctx.fill();ctx.fillStyle=e.hit?"#fff":e.color;ctx.beginPath();ctx.ellipse(0,0,16,13,0,0,Math.PI*2);ctx.fill();ctx.fillStyle=e.kind==="boss"?"#ff4d6d":e.kind==="spitter"?"#46f0ff":"#ffd43b";ctx.beginPath();ctx.arc(14,-9,9,0,Math.PI*2);ctx.fill();ctx.fillStyle="#ff922b";ctx.beginPath();ctx.moveTo(22,-9);ctx.lineTo(34,-4);ctx.lineTo(22,1);ctx.fill();ctx.fillStyle="#111";ctx.beginPath();ctx.arc(17,-11,2,0,Math.PI*2);ctx.fill();ctx.fillStyle=e.slow>0?"#9bf6ff":"#ff4d6d";ctx.fillRect(8,-23,12,6);ctx.strokeStyle="#ffb703";ctx.lineWidth=2;const st=Math.sin(frame/3+e.y*.01)*4;ctx.beginPath();ctx.moveTo(-7,11);ctx.lineTo(-10+st,22);ctx.moveTo(7,11);ctx.lineTo(10-st,22);ctx.stroke();ctx.restore();ctx.fillStyle="rgba(0,0,0,.45)";ctx.fillRect(e.x-e.r,e.y-e.r-12,e.r*2,4);ctx.fillStyle=e.slow>0?"#9bf6ff":"#7cffb2";ctx.fillRect(e.x-e.r,e.y-e.r-12,e.r*2*Math.max(0,e.hp/e.max),4)}
    function drawPlayer(){ctx.save();p.trail.forEach((t,i)=>{ctx.globalAlpha=t.life/70;ctx.fillStyle=i%2?"#46f0ff":"#7cffb2";ctx.beginPath();ctx.arc(t.x,t.y,p.r*(t.life/24),0,Math.PI*2);ctx.fill()});ctx.globalAlpha=1;ctx.translate(p.x,p.y);const pulse=1+Math.sin(frame/10)*.04;ctx.scale(pulse,pulse);ctx.shadowColor="#7cffb2";ctx.shadowBlur=22;ctx.fillStyle=p.inv>0&&frame%8<4?"#fff":"#101827";ctx.beginPath();for(let i=0;i<8;i++){const a=-Math.PI/2+i*Math.PI*2/8,rr=p.r+(i%2?5:0);ctx.lineTo(Math.cos(a)*rr,Math.sin(a)*rr)}ctx.closePath();ctx.fill();ctx.strokeStyle="#7cffb2";ctx.lineWidth=3;ctx.stroke();ctx.shadowBlur=10;ctx.fillStyle="#46f0ff";ctx.beginPath();ctx.arc(0,0,8+Math.sin(frame/7)*2,0,Math.PI*2);ctx.fill();ctx.strokeStyle="#ff54a0";ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,0,15,frame*.035,frame*.035+Math.PI*1.35);ctx.stroke();ctx.restore()}
    function drawDecor(){decor.forEach(d=>{if(d.x<cam.x-20||d.x>cam.x+W+20||d.y<cam.y-20||d.y>cam.y+H+20)return;if(d.k==="grass"){ctx.strokeStyle="rgba(22,77,38,.36)";ctx.beginPath();ctx.moveTo(d.x,d.y+d.s);ctx.lineTo(d.x+Math.sin(frame*.02+d.h)*3,d.y-d.s);ctx.stroke()}else if(d.k==="flower"){ctx.fillStyle="hsl("+d.h+",78%,68%)";ctx.beginPath();ctx.arc(d.x,d.y,d.s*.55,0,Math.PI*2);ctx.fill()}else if(d.k==="spark"){ctx.fillStyle="rgba(124,255,178,.22)";ctx.fillRect(d.x,d.y,d.s*2,2)}else{ctx.fillStyle="rgba(255,255,255,.035)";ctx.fillRect(d.x,d.y,d.s*3,3)}})}
    function draw(){ctx.save();if(shake>0){ctx.translate((Math.random()-.5)*shake,(Math.random()-.5)*shake);shake*=.86}ctx.fillStyle="#4f9f4a";ctx.fillRect(0,0,W,H);ctx.fillStyle="rgba(255,255,255,.05)";for(let i=0;i<95;i++){const x=(i*97+frame*.12)%W,y=(i*53)%H;ctx.fillRect(x,y,18,3)}ctx.strokeStyle="rgba(31,92,38,.35)";for(let x=-40;x<W+40;x+=40){ctx.beginPath();ctx.moveTo(x+frame%40,0);ctx.lineTo(x+frame%40,H);ctx.stroke()}for(let y=-40;y<H+40;y+=40){ctx.beginPath();ctx.moveTo(0,y+frame%40);ctx.lineTo(W,y+frame%40);ctx.stroke()}obstacles.forEach(drawObstacle);if(lv("aura")){ctx.fillStyle="rgba(124,255,178,.10)";ctx.beginPath();ctx.arc(p.x,p.y,38+lv("aura")*13,0,Math.PI*2);ctx.fill()}gems.forEach(g=>{ctx.fillStyle="#46f0ff";ctx.beginPath();ctx.arc(g.x,g.y,g.r,0,Math.PI*2);ctx.fill()});drops.forEach(d=>{ctx.fillStyle=d.k==="heart"?"#ff4d6d":"#ffe66d";ctx.beginPath();ctx.arc(d.x,d.y,d.r,0,Math.PI*2);ctx.fill()});bullets.forEach(b=>{ctx.fillStyle=b.c;ctx.shadowColor=b.c;ctx.shadowBlur=12;ctx.beginPath();ctx.arc(b.x,b.y,b.r,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0});enemies.slice().sort((a,b)=>a.y-b.y).forEach(drawChicken);for(let i=0;i<lv("orbit");i++){const a=frame/28+i*Math.PI*2/lv("orbit");ctx.fillStyle="#ffe66d";ctx.beginPath();ctx.arc(p.x+Math.cos(a)*(48+lv("orbit")*5),p.y+Math.sin(a)*(48+lv("orbit")*5),9,0,Math.PI*2);ctx.fill()}drawPlayer();parts.forEach(o=>{if(o.laser){ctx.strokeStyle="rgba(70,240,255,"+o.life/10+")";ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(o.x1,o.y1);ctx.lineTo(o.x2,o.y2);ctx.stroke()}else{ctx.fillStyle=o.c;ctx.globalAlpha=Math.max(0,o.life/26);ctx.beginPath();ctx.arc(o.x,o.y,3,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1}});texts.forEach(t=>{ctx.fillStyle="rgba(255,255,255,"+t.life/26+")";ctx.font="900 13px Inter, Arial";ctx.fillText(t.v,t.x,t.y)});ctx.restore()}
    function draw(){ctx.save();if(shake>0){ctx.translate((Math.random()-.5)*shake,(Math.random()-.5)*shake);shake*=.86}let sky=ctx.createLinearGradient(0,0,W,H);sky.addColorStop(0,"#163b52");sky.addColorStop(.48,"#4f9f4a");sky.addColorStop(1,"#2f6b43");ctx.fillStyle=sky;ctx.fillRect(0,0,W,H);ctx.save();ctx.translate(-cam.x,-cam.y);let ground=ctx.createLinearGradient(cam.x,cam.y,cam.x+W,cam.y+H);ground.addColorStop(0,"#4f9f4a");ground.addColorStop(.45,"#3f8f55");ground.addColorStop(1,"#365d83");ctx.fillStyle=ground;ctx.fillRect(cam.x,cam.y,W,H);drawDecor();ctx.fillStyle="rgba(255,255,255,.05)";for(let i=0;i<240;i++){const x=(i*173)%WORLD_W,y=(i*89)%WORLD_H;if(x>cam.x-30&&x<cam.x+W+30&&y>cam.y-30&&y<cam.y+H+30)ctx.fillRect(x,y,18,3)}ctx.strokeStyle="rgba(31,92,38,.35)";for(let x=Math.floor(cam.x/80)*80;x<cam.x+W+80;x+=80){ctx.beginPath();ctx.moveTo(x,cam.y);ctx.lineTo(x,cam.y+H);ctx.stroke()}for(let y=Math.floor(cam.y/80)*80;y<cam.y+H+80;y+=80){ctx.beginPath();ctx.moveTo(cam.x,y);ctx.lineTo(cam.x+W,y);ctx.stroke()}obstacles.forEach(o=>{if(o.x+o.w>cam.x-50&&o.x<cam.x+W+50&&o.y+o.h>cam.y-50&&o.y<cam.y+H+50)drawObstacle(o)});if(lv("aura")){ctx.fillStyle="rgba(124,255,178,.11)";ctx.beginPath();ctx.arc(p.x,p.y,40+lv("aura")*15,0,Math.PI*2);ctx.fill();ctx.strokeStyle="rgba(124,255,178,.32)";ctx.stroke()}gems.forEach(g=>{ctx.shadowColor="#46f0ff";ctx.shadowBlur=10;ctx.fillStyle="#46f0ff";ctx.beginPath();ctx.moveTo(g.x,g.y-g.r);ctx.lineTo(g.x+g.r,g.y);ctx.lineTo(g.x,g.y+g.r);ctx.lineTo(g.x-g.r,g.y);ctx.closePath();ctx.fill();ctx.shadowBlur=0});drops.forEach(d=>{ctx.fillStyle=d.k==="heart"?"#ff4d6d":"#ffe66d";ctx.beginPath();ctx.arc(d.x,d.y,d.r,0,Math.PI*2);ctx.fill()});bullets.forEach(b=>{ctx.fillStyle=b.c;ctx.shadowColor=b.c;ctx.shadowBlur=14;ctx.beginPath();ctx.arc(b.x,b.y,b.r,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0});enemies.slice().sort((a,b)=>a.y-b.y).forEach(drawChicken);for(let i=0;i<lv("orbit");i++){const a=frame/27+i*Math.PI*2/lv("orbit");ctx.shadowColor="#ffe66d";ctx.shadowBlur=16;ctx.fillStyle="#ffe66d";ctx.beginPath();ctx.arc(p.x+Math.cos(a)*(50+lv("orbit")*5),p.y+Math.sin(a)*(50+lv("orbit")*5),9,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0}for(let i=0;i<lv("drone");i++){const a=frame/28+i*Math.PI*2/Math.max(1,lv("drone"));ctx.fillStyle="#c77dff";ctx.beginPath();ctx.arc(p.x+Math.cos(a)*42,p.y+Math.sin(a)*42,6,0,Math.PI*2);ctx.fill()}drawPlayer();parts.forEach(o=>{if(o.laser){ctx.strokeStyle=o.c==="#f8f7ff"?"rgba(248,247,255,"+o.life/14+")":"rgba(70,240,255,"+o.life/14+")";ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(o.x1,o.y1);ctx.lineTo(o.x2,o.y2);ctx.stroke()}else if(o.ring){ctx.strokeStyle=o.c;ctx.globalAlpha=Math.max(0,o.life/28);ctx.lineWidth=4;ctx.beginPath();ctx.arc(o.x,o.y,o.r+(o.max-o.r)*(1-o.life/28),0,Math.PI*2);ctx.stroke();ctx.globalAlpha=1}else{ctx.fillStyle=o.c;ctx.globalAlpha=Math.max(0,o.life/32);ctx.beginPath();ctx.arc(o.x,o.y,o.r||3,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1}});texts.forEach(t=>{ctx.fillStyle="rgba(255,255,255,"+t.life/26+")";ctx.font="900 13px Inter, Arial";ctx.fillText(t.v,t.x,t.y)});ctx.restore();ctx.fillStyle="rgba(0,0,0,.32)";ctx.fillRect(14,14,172,8);ctx.fillStyle="#7cffb2";ctx.fillRect(14,14,172*(p.x/WORLD_W),8);ctx.fillStyle="rgba(0,0,0,.32)";ctx.fillRect(14,28,8,108);ctx.fillStyle="#46f0ff";ctx.fillRect(14,28+108*(1-p.y/WORLD_H),8,108*(p.y/WORLD_H));ctx.restore()}
    function loop(){if(state==="play"){frame++;seconds=frame/60;if(frame%(seconds<30?88:seconds<70?70:Math.max(22,62-Math.floor(seconds/10)))===0)wave();update();fire();shots();mobs();pickups();parts.forEach(o=>{o.life--;if(!o.laser&&!o.ring){o.x+=o.vx||0;o.y+=o.vy||0}});parts=parts.filter(o=>o.life>0);texts.forEach(t=>{t.life--;t.y-=.45});texts=texts.filter(t=>t.life>0);if(frame%60===0)score+=Math.floor(2+seconds/40);hud()}draw();requestAnimationFrame(loop)}
    ...
    startBtn.onclick=start;saveBtn.onclick=saveScore;restartBtn.onclick=start;musicBtn.onclick=()=>{musicOn=!musicOn;musicBtn.textContent=musicOn?"Musik: An":"Musik: Aus";if(!musicOn)stopMusic();else if(state==="play")startMusic()};saveBtn.style.display="none";restartBtn.style.display="none";reset();renderScores();loop();
    </script></body></html>
