...
# =========================
    ...
    class SurvivorScene extends Phaser.Scene{
      ...
      create(){
        ...
        this.bossHud=this.add.graphics().setScrollFactor(0).setDepth(96);
        this.bossHudText=this.add.text(500,18,"",{font:"900 18px Inter,Arial",color:"#ffffff",stroke:"#350006",strokeThickness:5}).setOrigin(.5,0).setScrollFactor(0).setDepth(97);
      ...
      enemySpecial(e){if(this.time.now<e.nextSpecial)return;e.nextSpecial=this.time.now+(e.kind==="boss"?1700:2600);if(e.kind==="healer"){this.telegraph(e.x,e.y,130,0x7cffb2,360,()=>{if(!e.active)return;this.enemies.getChildren().forEach(o=>{if(o!==e&&Phaser.Math.Distance.Between(o.x,o.y,e.x,e.y)<130){o.hp=Math.min(o.max,o.hp+o.max*.12);this.updateEnemyBar(o)}})})}if(e.kind==="spitter"){const tx=this.player.x,ty=this.player.y;this.telegraph(tx,ty,76,0x46f0ff,520,()=>{if(e.active&&state==="play"&&Phaser.Math.Distance.Between(tx,ty,this.player.x,this.player.y)<76&&p.inv<=0)this.playerDamage(e,Phaser.Math.Angle.Between(e.x,e.y,this.player.x,this.player.y))})}if(e.kind==="boss")this.bossSpecial(e)}
      beginBossFight(e){currentBoss=e;this.enemies.getChildren().slice().forEach(o=>{if(o!==e){this.destroyEnemyBar(o);this.burst(o.x,o.y,0xff4d6d,8);o.destroy()}});this.bullets.clear(true,true);this.cameras.main.flash(380,255,20,30);this.cameras.main.shake(650,.018);this.ring(e.x,e.y,900,0xff1f3d);const warning=this.add.text(500,245,"BOSS ERSCHEINT",{font:"900 58px Inter,Arial",color:"#ff334f",stroke:"#180006",strokeThickness:12,align:"center"}).setOrigin(.5).setScrollFactor(0).setDepth(210).setScale(.45);this.tweens.add({targets:warning,scale:1.08,duration:320,ease:"Back.Out",hold:950,yoyo:true,onComplete:()=>warning.destroy()});this.drawBossHud();hud()}
      bossSpecial(e){if(!e.active||e.dead)return;const attack=e.bossAttack++%3,phase=e.phase||0;if(attack===0){const tx=this.player.x,ty=this.player.y,r=150+phase*24;this.telegraph(tx,ty,r,0xff334f,650,()=>{if(e.active&&state==="play"&&Phaser.Math.Distance.Between(tx,ty,this.player.x,this.player.y)<r&&p.inv<=0)this.playerDamage(e,Phaser.Math.Angle.Between(e.x,e.y,this.player.x,this.player.y))})}else if(attack===1){const r=260+phase*45;this.telegraph(e.x,e.y,r,0xff1010,760,()=>{if(!e.active||state!=="play")return;this.cameras.main.shake(260,.012);if(Phaser.Math.Distance.Between(e.x,e.y,this.player.x,this.player.y)<r&&p.inv<=0)this.playerDamage(e,Phaser.Math.Angle.Between(e.x,e.y,this.player.x,this.player.y))})}else{const tx=this.player.x,ty=this.player.y;this.line(e.x,e.y,tx,ty,0xff334f);this.telegraph(tx,ty,105+phase*12,0xff8a00,520,()=>{if(!e.active||state!=="play")return;e.setPosition(tx,ty);this.ring(tx,ty,150,0xff8a00);this.cameras.main.shake(220,.011);if(Phaser.Math.Distance.Between(tx,ty,this.player.x,this.player.y)<145&&p.inv<=0)this.playerDamage(e,Phaser.Math.Angle.Between(e.x,e.y,this.player.x,this.player.y))})}}
      ...
      bossPhase(e){if(currentBoss!==e||e.dead)return;const pct=e.hp/e.max,next=pct<.25?3:pct<.5?2:pct<.75?1:0;if((e.phase||0)<next){e.phase=next;e.base+=14;e.nextSpecial=this.time.now+650;this.cameras.main.flash(180,255,20,30);this.cameras.main.shake(260,.012);this.ring(e.x,e.y,310+next*45,0xff1f3d)}}
      ...
      drawBossHud(){if(!this.bossHud||!this.bossHudText)return;this.bossHud.clear();if(!currentBoss||!currentBoss.active){this.bossHudText.setText("");return}const pct=Phaser.Math.Clamp(currentBoss.hp/currentBoss.max,0,1),x=165,y=48,w=670,h=20;this.bossHud.fillStyle(0x160005,.92).fillRoundedRect(x-5,y-5,w+10,h+10,9);this.bossHud.lineStyle(3,0xff6677,.95).strokeRoundedRect(x-5,y-5,w+10,h+10,9);this.bossHud.fillStyle(0x8a0018,1).fillRoundedRect(x,y,w,h,5);this.bossHud.fillStyle(0xff1838,1).fillRoundedRect(x,y,Math.max(2,w*pct),h,5);this.bossHudText.setText("MEGA CHICKEN  "+Math.ceil(currentBoss.hp)+" / "+Math.ceil(currentBoss.max)+" HP")}
