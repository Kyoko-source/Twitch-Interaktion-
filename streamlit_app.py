...
# =========================
    ...
    let scene=null,state="menu",p=null,score=0,kills=0,seconds=0,saved=false,lastBoss=0,musicOn=true,audioCtx=null;
    ...
    class SurvivorScene extends Phaser.Scene{
      ...
      resetRun(){p={hp:104,max:104,xp:0,need:34,level:1,speed:245,inv:0,up:{bolt:1},tim:{bolt:0,bomb:0,laser:0,drone:0,thunder:0,nova:3.8}};score=0;kills=0;seconds=0;saved=false;lastBoss=0;state="play";this.player.setPosition(WORLD_W/2,WORLD_H/2);this.player.body.enable=true;this.enemies.clear(true,true);this.bullets.clear(true,true);this.gems.clear(true,true);this.drops.clear(true,true);this.timeStart=this.time.now;hud()}
      ...
      mobs(dt){this.enemies.getChildren().forEach(e=>{e.slow=Math.max(0,e.slow-dt);const a=Phaser.Math.Angle.Between(e.x,e.y,this.player.x,this.player.y),speed=e.base*(e.slow>0?.55:1);e.setVelocity(Math.cos(a)*speed,Math.sin(a)*speed);e.rotation=Math.sin(this.time.now*.008+e.x)*.12;if(this.lv("aura")&&Phaser.Math.Distance.Between(e.x,e.y,this.player.x,this.player.y)<40+this.lv("aura")*15)this.hit(e,.22*this.lv("aura")*(1+this.lv("might")*.16));if(Phaser.Math.Distance.Between(e.x,e.y,this.player.x,this.player.y)<e.r+20&&p.inv<=0){const block=1-this.lv("shield")*.08;p.hp-=(e.kind==="boss"?28:e.kind==="tank"?16:e.kind==="runner"?11:10)*Math.max(.55,block);p.inv=430;sfx("hurt");this.cameras.main.shake(140,.006);if(p.hp<=0)this.over();hud()}})}
      ...
      hit(e,d){if(!e.active)return;e.hp-=d;if(this.lv("frost")&&Math.random()<.55){e.slow=900+this.lv("frost")*160;e.setTint(0x9bf6ff)}if(Math.random()<.08)sfx("hit");if(e.hp<=0)this.kill(e)}
      kill(e){sfx("kill");kills++;score+=Math.floor(e.score+seconds*.8+p.level*2);this.blood(e.x,e.y,e.kind==="boss"||e.kind==="elite");this.burst(e.x,e.y,e.tintTopLeft||0xffffff,e.kind==="boss"?38:14);this.spawnGem(e.x,e.y,e.xp,e.kind==="boss"?1.35:1);if(Math.random()<.035||e.kind==="elite")this.spawnDrop(e.x,e.y,"heart");if(Math.random()<.022||e.kind==="boss")this.spawnDrop(e.x,e.y,"magnet");e.destroy();hud()}
    ...
    function start(){if(!scene)return;audio();sfx("start");startMusic();overlay.style.display="none";scene.scene.resume();scene.resetRun()}
    ...
    document.addEventListener("keydown",e=>{if(["arrowup","arrowdown","arrowleft","arrowright"," "].includes(e.key.toLowerCase()))e.preventDefault()});
