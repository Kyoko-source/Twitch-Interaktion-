...
# =========================
    ...
    let scene=null,state="menu",p=null,score=0,kills=0,seconds=0,saved=false,lastBoss=0,currentBoss=null,musicOn=true,audioCtx=null,moveKeys={},meta=loadMeta();
    ...
    class SurvivorScene extends Phaser.Scene{
      ...
      resetRun(){if(this.enemies)this.enemies.getChildren().forEach(e=>this.destroyEnemyBar(e));const mhp=104+meta.hp*10;p={hp:mhp,max:mhp,xp:0,need:34,level:1,speed:245+meta.spd*9,inv:0,up:{bolt:1},tim:{bolt:0,bomb:0,laser:0,drone:0,thunder:0,nova:3.8}};score=0;kills=0;seconds=0;saved=false;lastBoss=0;currentBoss=null;state="play";this.player.setPosition(WORLD_W/2,WORLD_H/2);this.player.body.enable=true;this.enemies.clear(true,true);this.bullets.clear(true,true);this.gems.clear(true,true);this.drops.clear(true,true);this.timeStart=this.time.now;hud()}
      lv(id){return p.up[id]||0}
      evolved(id){return this.lv(id)>=(evo[id]||999)}
      update(t,dt){if(state!=="play")return;seconds=(t-this.timeStart)/1000;this.move(dt);this.wave(t);this.fire(dt);this.mobs(dt);this.separateEnemies();this.effects(t);this.drawMiniMap();if(t%1000<17){score+=Math.floor(2+seconds/40);p.hp=Math.min(p.max,p.hp+.18+this.lv("regen")*.6);hud()}}
      ...
      separateEnemies(){const a=this.enemies.getChildren();for(let i=0;i<a.length;i++)for(let j=i+1;j<a.length;j++){const e=a[i],o=a[j],d=Phaser.Math.Distance.Between(e.x,e.y,o.x,o.y),min=(e.r+o.r)*.72;if(d>0&&d<min){const ang=Phaser.Math.Angle.Between(o.x,o.y,e.x,e.y),push=(min-d)*.035;e.x+=Math.cos(ang)*push;o.x-=Math.cos(ang)*push;e.y+=Math.sin(ang)*push;o.y-=Math.sin(ang)*push}}}
      ...
      damageText(x,y,v,crit){const t=this.add.text(x,y,(crit?"CRIT ":"")+v,{font:"900 16px Inter,Arial",color:crit?"#ffe66d":"#ffffff",stroke:"#061015",strokeThickness:4}).setDepth(90).setOrigin(.5);this.tweens.add({targets:t,y:y-32,alpha:0,scale:crit?1.35:1,duration:520,onComplete:()=>t.destroy()})}
