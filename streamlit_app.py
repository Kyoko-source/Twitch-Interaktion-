...
# =========================
    ...
    let state="menu",p,enemies,bullets,gems,drops,parts,texts,obstacles,frame=0,seconds=0,score=0,kills=0,saved=false,lastBoss=0,shake=0,audioCtx=null,lastCluck=0;
    const pool=[
    ...
    function start(){audio();sfx("start");reset();state="play";overlay.style.display="none"}
    ...
    function spawn(kind="grunt"){const s=Math.floor(Math.random()*4),pos=[{x:-35,y:Math.random()*H},{x:W+35,y:Math.random()*H},{x:Math.random()*W,y:-35},{x:Math.random()*W,y:H+35}][s],m=seconds/60;
      const t={grunt:[42+m*20,1.35+m*.075,15,7,12,"#f8f7ff"],runner:[30+m*15,2.18+m*.08,12,8,16,"#ffe66d"],tank:[130+m*42,.92+m*.04,22,18,34,"#ff9f1c"],elite:[280+m*78,1.22+m*.04,26,38,95,"#ff54a0"],boss:[1180+m*330,.96,42,130,450,"#c77dff"]}[kind];
      enemies.push({...pos,kind,hp:t[0],max:t[0],speed:t[1],r:t[2],xp:t[3],score:t[4],color:t[5],hit:0})}
    function wave(){const n=Math.min(18,3+Math.floor(seconds/16));for(let i=0;i<n;i++){const r=Math.random(); if(seconds>75&&r<.15)spawn("tank"); else if(seconds>25&&r<.42)spawn("runner"); else spawn("grunt")} if(seconds>42&&frame%300===0)spawn("elite");const bm=Math.floor(seconds/150);if(bm>0&&bm!==lastBoss){lastBoss=bm;sfx("boss");spawn("boss")}}
    ...
    function fire(){const cd=Math.max(.52,1-lv("cooldown")*.065),might=1+lv("might")*.15;p.tim.bolt--;if(p.tim.bolt<=0&&enemies.length){const t=nearest(),amt=lv("bolt");for(let i=0;i<amt;i++){const a=Math.atan2(t.y-p.y,t.x-p.x)+(i-(amt-1)/2)*.16;bullets.push({x:p.x,y:p.y,vx:Math.cos(a)*7.2,vy:Math.sin(a)*7.2,r:5,d:14*might,life:74,k:"bolt",c:"#7cffb2"})}p.tim.bolt=Math.max(18,(52-lv("bolt")*3)*cd)}
      ...
      if(lv("laser")>0){p.tim.laser--;if(p.tim.laser<=0&&enemies.length){const t=enemies.slice().sort((a,b)=>b.hp-a.hp)[0];hit(t,(58+lv("laser")*30)*might,true);parts.push({x1:p.x,y1:p.y,x2:t.x,y2:t.y,life:10,laser:true});p.tim.laser=Math.max(88,(220-lv("laser")*17)*cd)}}}
