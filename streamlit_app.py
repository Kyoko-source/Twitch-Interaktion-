...
# =========================
    ...
    const W=canvas.width,H=canvas.height,WORLD_W=2600,WORLD_H=1800,keys={};
    let state="menu",p,enemies,bullets,gems,drops,parts,texts,obstacles,cam={x:0,y:0},frame=0,seconds=0,score=0,kills=0,saved=false,lastBoss=0,shake=0,audioCtx=null,lastCluck=0,musicOn=true;
    const pool=[
    ...
    function sfx(k,n=1){try{if(k==="start"){tone(420,.07);tone(640,.08,"triangle",.04,.08);tone(860,.1,"triangle",.04,.16)}if(k==="shoot"){for(let i=0;i<Math.min(n,5);i++)tone(760+Math.random()*260,.026,"square",.012,i*.012)}if(k==="bomb")tone(145,.09,"sawtooth",.038);if(k==="laser"){tone(980,.045,"triangle",.035);tone(1480,.06,"sine",.025,.025)}if(k==="hit")tone(520+Math.random()*160,.035,"square",.018);if(k==="kill"){tone(720,.045,"triangle",.026);if(Math.random()<.16){tone(360,.045,"square",.03,.03);tone(235,.07,"sawtooth",.025,.075)}}if(k==="hurt"){tone(180,.14,"sawtooth",.055);tone(95,.18,"square",.035,.06)}if(k==="level"){tone(523,.08);tone(659,.08,"triangle",.045,.08);tone(784,.12,"triangle",.045,.16)}if(k==="boss"){tone(90,.35,"sawtooth",.06);tone(140,.28,"square",.04,.08)}}catch(e){}}
    function makeObstacles(){obstacles=[{x:360,y:260,w:94,h:64,k:"house"},{x:1280,y:320,w:112,h:72,k:"house"},{x:1960,y:280,w:118,h:74,k:"house"},{x:640,y:760,w:118,h:54,k:"stone"},{x:1680,y:980,w:150,h:58,k:"stone"},{x:1040,y:1180,w:98,h:70,k:"house"},{x:2180,y:1260,w:138,h:76,k:"house"},{x:520,y:1340,w:90,h:52,k:"stone"},{x:1430,y:1460,w:132,h:56,k:"stone"},{x:910,y:520,w:82,h:44,k:"stone"},{x:2240,y:700,w:92,h:48,k:"stone"},{x:300,y:1040,w:120,h:66,k:"house"}]}
    ...
    function reset(){p={x:WORLD_W/2,y:WORLD_H/2,r:17,hp:82,max:82,xp:0,need:34,level:1,speed:3.05,inv:0,up:{bolt:1},tim:{bolt:0,bomb:0,laser:0}};cam={x:p.x-W/2,y:p.y-H/2};enemies=[];bullets=[];gems=[];drops=[];parts=[];texts=[];makeObstacles();frame=0;seconds=0;score=0;kills=0;saved=false;lastBoss=0;shake=0;lastCluck=0;hud()}
    ...
    function spawn(kind="grunt"){const s=Math.floor(Math.random()*4),left=cam.x-70,right=cam.x+W+70,top=cam.y-70,bottom=cam.y+H+70,pos=[{x:left,y:cam.y+Math.random()*H},{x:right,y:cam.y+Math.random()*H},{x:cam.x+Math.random()*W,y:top},{x:cam.x+Math.random()*W,y:bottom}][s],m=seconds/60;pos.x=clamp(pos.x,24,WORLD_W-24);pos.y=clamp(pos.y,24,WORLD_H-24);
      const t={grunt:[34+m*16,1.18+m*.065,15,7,12,"#f8f7ff"],runner:[26+m*13,1.85+m*.075,12,8,16,"#ffe66d"],tank:[118+m*38,.84+m*.04,22,18,34,"#ff9f1c"],elite:[250+m*70,1.12+m*.04,26,38,95,"#ff54a0"],boss:[1080+m*310,.92,42,130,450,"#c77dff"]}[kind];
      enemies.push({...pos,kind,hp:t[0],max:t[0],speed:t[1],r:t[2],xp:t[3],score:t[4],color:t[5],hit:0})}
    function wave(){const n=seconds<25?1:seconds<55?2:Math.min(13,3+Math.floor((seconds-55)/18));for(let i=0;i<n;i++){const r=Math.random(); if(seconds>90&&r<.12)spawn("tank"); else if(seconds>38&&r<.32)spawn("runner"); else spawn("grunt")} if(seconds>70&&frame%360===0)spawn("elite");const bm=Math.floor(seconds/165);if(bm>0&&bm!==lastBoss){lastBoss=bm;sfx("boss");spawn("boss")}}
    ...
    function fire(){const cd=Math.max(.52,1-lv("cooldown")*.065),might=1+lv("might")*.15;p.tim.bolt--;if(p.tim.bolt<=0&&enemies.length){const t=nearest(),amt=lv("bolt");sfx("shoot",amt);for(let i=0;i<amt;i++){const a=Math.atan2(t.y-p.y,t.x-p.x)+(i-(amt-1)/2)*.16;bullets.push({x:p.x,y:p.y,vx:Math.cos(a)*7.2,vy:Math.sin(a)*7.2,r:5,d:14*might,life:74,k:"bolt",c:"#7cffb2"})}p.tim.bolt=Math.max(18,(52-lv("bolt")*3)*cd)}
      if(lv("bomb")>0){p.tim.bomb--;if(p.tim.bomb<=0&&enemies.length){const t=nearest(),a=Math.atan2(t.y-p.y,t.x-p.x);sfx("bomb");bullets.push({x:p.x,y:p.y,vx:Math.cos(a)*4.5,vy:Math.sin(a)*4.5,r:9,d:(42+lv("bomb")*18)*might,life:86,k:"bomb",c:"#ffe66d"});p.tim.bomb=Math.max(42,(132-lv("bomb")*11)*cd)}}
      if(lv("laser")>0){p.tim.laser--;if(p.tim.laser<=0&&enemies.length){const t=enemies.slice().sort((a,b)=>b.hp-a.hp)[0];sfx("laser");hit(t,(58+lv("laser")*30)*might,true);parts.push({x1:p.x,y1:p.y,x2:t.x,y2:t.y,life:10,laser:true});p.tim.laser=Math.max(88,(220-lv("laser")*17)*cd)}}}
    function hit(e,d,boom=false){if(!e)return;e.hp-=d;e.hit=5;if(Math.random()<.11)sfx("hit");if(Math.random()<.18)texts.push({x:e.x,y:e.y-e.r,v:Math.floor(d),life:26});if(e.hp<=0)kill(e);if(boom)shake=Math.min(8,shake+2)}
    function kill(e){const i=enemies.indexOf(e);if(i>=0)enemies.splice(i,1);sfx("kill");kills++;score+=Math.floor(e.score+seconds*.8+p.level*2);gems.push({x:e.x,y:e.y,v:e.xp,r:e.kind==="boss"?9:6});if(Math.random()<.026||e.kind==="elite")drops.push({x:e.x,y:e.y,k:"heart",r:8});if(Math.random()<.018||e.kind==="boss")drops.push({x:e.x,y:e.y,k:"magnet",r:8});for(let j=0;j<10;j++)parts.push({x:e.x,y:e.y,vx:(Math.random()-.5)*5,vy:(Math.random()-.5)*5,life:26,c:e.color})}
    ...
    function update(){let dx=0,dy=0;if(keys.arrowleft||keys.a)dx--;if(keys.arrowright||keys.d)dx++;if(keys.arrowup||keys.w)dy--;if(keys.arrowdown||keys.s)dy++;const l=Math.hypot(dx,dy)||1,ox=p.x,oy=p.y;p.x=clamp(p.x+dx/l*p.speed,p.r,WORLD_W-p.r);obstacles.forEach(o=>pushOut(o,p));p.y=clamp(p.y+dy/l*p.speed,p.r,WORLD_H-p.r);obstacles.forEach(o=>pushOut(o,p));cam.x=clamp(p.x-W/2,0,WORLD_W-W);cam.y=clamp(p.y-H/2,0,WORLD_H-H);p.inv=Math.max(0,p.inv-1);if(frame%60===0)p.hp=Math.min(p.max,p.hp+.16+lv("regen")*.55)}
    function mobs(){const aura=lv("aura"),range=38+aura*13,orbit=lv("orbit");enemies.slice().forEach(e=>{const a=Math.atan2(p.y-e.y,p.x-e.x),oldx=e.x,oldy=e.y;e.x+=Math.cos(a)*e.speed;e.y+=Math.sin(a)*e.speed;obstacles.forEach(o=>{if(pushOut(o,e)){e.x+=(Math.random()-.5)*1.8;e.y+=(Math.random()-.5)*1.8}});e.hit=Math.max(0,e.hit-1);if(aura&&Math.hypot(e.x-p.x,e.y-p.y)<range)hit(e,.20*aura*(1+lv("might")*.15));if(orbit){for(let i=0;i<orbit;i++){const oa=frame/28+i*Math.PI*2/orbit,ox=p.x+Math.cos(oa)*(48+orbit*5),oy=p.y+Math.sin(oa)*(48+orbit*5);if(Math.hypot(e.x-ox,e.y-oy)<e.r+9)hit(e,.48*orbit*(1+lv("might")*.15))}} if(Math.hypot(e.x-p.x,e.y-p.y)<e.r+p.r&&p.inv<=0){p.hp-=e.kind==="boss"?27:e.kind==="tank"?16:e.kind==="runner"?11:10;sfx("hurt");p.inv=28;shake=8;if(p.hp<=0)over()}})}
    ...
    function shots(){bullets.slice().forEach(b=>{b.x+=b.vx;b.y+=b.vy;b.life--;if(obstacles.some(o=>circleRect(o,b.x,b.y,b.r))){b.life=0;parts.push({x:b.x,y:b.y,vx:0,vy:0,life:12,c:"#ffffff"});return}enemies.slice().forEach(e=>{if(Math.hypot(e.x-b.x,e.y-b.y)<e.r+b.r){hit(e,b.d);if(b.k==="bomb"){enemies.slice().forEach(o=>{if(Math.hypot(o.x-b.x,o.y-b.y)<86)hit(o,b.d*.55)});shake=Math.min(9,shake+4)}b.life=0}})});bullets=bullets.filter(b=>b.life>0&&b.x>-40&&b.x<WORLD_W+40&&b.y>-40&&b.y<WORLD_H+40)}
    function drawObstacle(o){ctx.save();ctx.shadowColor="rgba(0,0,0,.22)";ctx.shadowBlur=10;ctx.shadowOffsetY=6;if(o.k==="house"){ctx.fillStyle="#8b5a32";ctx.fillRect(o.x+8,o.y+22,o.w-16,o.h-18);ctx.fillStyle="#5b2f27";ctx.beginPath();ctx.moveTo(o.x,o.y+28);ctx.lineTo(o.x+o.w/2,o.y);ctx.lineTo(o.x+o.w,o.y+28);ctx.closePath();ctx.fill();ctx.fillStyle="#ffe2a8";ctx.fillRect(o.x+o.w*.18,o.y+36,18,16);ctx.fillStyle="#2b1b18";ctx.fillRect(o.x+o.w*.62,o.y+38,18,30)}else{ctx.fillStyle="#7b8490";ctx.beginPath();ctx.ellipse(o.x+o.w*.5,o.y+o.h*.55,o.w*.5,o.h*.45,0,0,Math.PI*2);ctx.fill();ctx.fillStyle="rgba(255,255,255,.18)";ctx.beginPath();ctx.ellipse(o.x+o.w*.34,o.y+o.h*.35,o.w*.18,o.h*.12,-.4,0,Math.PI*2);ctx.fill()}ctx.restore()}
    function drawChicken(e){ctx.save();ctx.translate(e.x,e.y);const bob=Math.sin(frame/5+e.x*.01)*2,scale=e.r/16;ctx.scale(scale,scale);ctx.translate(0,bob);ctx.fillStyle="rgba(0,0,0,.24)";ctx.beginPath();ctx.ellipse(4,17,18,5,0,0,Math.PI*2);ctx.fill();ctx.fillStyle=e.hit?"#fff":e.color;ctx.beginPath();ctx.ellipse(0,0,16,13,0,0,Math.PI*2);ctx.fill();ctx.fillStyle=e.kind==="boss"?"#ff4d6d":"#ffd43b";ctx.beginPath();ctx.arc(14,-9,9,0,Math.PI*2);ctx.fill();ctx.fillStyle="#ff922b";ctx.beginPath();ctx.moveTo(22,-9);ctx.lineTo(34,-4);ctx.lineTo(22,1);ctx.fill();ctx.fillStyle="#111";ctx.beginPath();ctx.arc(17,-11,2,0,Math.PI*2);ctx.fill();ctx.fillStyle="#ff4d6d";ctx.fillRect(8,-23,12,6);ctx.strokeStyle="#ffb703";ctx.lineWidth=2;const st=Math.sin(frame/3+e.y*.01)*4;ctx.beginPath();ctx.moveTo(-7,11);ctx.lineTo(-10+st,22);ctx.moveTo(7,11);ctx.lineTo(10-st,22);ctx.stroke();ctx.restore();ctx.fillStyle="rgba(0,0,0,.45)";ctx.fillRect(e.x-e.r,e.y-e.r-12,e.r*2,4);ctx.fillStyle="#7cffb2";ctx.fillRect(e.x-e.r,e.y-e.r-12,e.r*2*Math.max(0,e.hp/e.max),4)}
    function drawPlayer(){ctx.save();ctx.translate(p.x,p.y);ctx.fillStyle=p.inv>0&&frame%8<4?"#fff":"#7cffb2";ctx.beginPath();ctx.arc(0,0,p.r,0,Math.PI*2);ctx.fill();ctx.fillStyle="#061015";ctx.beginPath();ctx.arc(-5,-4,3,0,Math.PI*2);ctx.arc(6,-4,3,0,Math.PI*2);ctx.fill();ctx.fillStyle="#46f0ff";ctx.fillRect(-11,-20,22,6);ctx.restore()}
    ...
    function draw(){ctx.save();if(shake>0){ctx.translate((Math.random()-.5)*shake,(Math.random()-.5)*shake);shake*=.86}ctx.fillStyle="#4f9f4a";ctx.fillRect(0,0,W,H);ctx.save();ctx.translate(-cam.x,-cam.y);ctx.fillStyle="#4f9f4a";ctx.fillRect(cam.x,cam.y,W,H);ctx.fillStyle="rgba(255,255,255,.05)";for(let i=0;i<240;i++){const x=(i*173)%WORLD_W,y=(i*89)%WORLD_H;if(x>cam.x-30&&x<cam.x+W+30&&y>cam.y-30&&y<cam.y+H+30)ctx.fillRect(x,y,18,3)}ctx.strokeStyle="rgba(31,92,38,.35)";for(let x=Math.floor(cam.x/40)*40;x<cam.x+W+40;x+=40){ctx.beginPath();ctx.moveTo(x,cam.y);ctx.lineTo(x,cam.y+H);ctx.stroke()}for(let y=Math.floor(cam.y/40)*40;y<cam.y+H+40;y+=40){ctx.beginPath();ctx.moveTo(cam.x,y);ctx.lineTo(cam.x+W,y);ctx.stroke()}obstacles.forEach(o=>{if(o.x+o.w>cam.x-40&&o.x<cam.x+W+40&&o.y+o.h>cam.y-40&&o.y<cam.y+H+40)drawObstacle(o)});if(lv("aura")){ctx.fillStyle="rgba(124,255,178,.10)";ctx.beginPath();ctx.arc(p.x,p.y,38+lv("aura")*13,0,Math.PI*2);ctx.fill()}gems.forEach(g=>{ctx.fillStyle="#46f0ff";ctx.beginPath();ctx.arc(g.x,g.y,g.r,0,Math.PI*2);ctx.fill()});drops.forEach(d=>{ctx.fillStyle=d.k==="heart"?"#ff4d6d":"#ffe66d";ctx.beginPath();ctx.arc(d.x,d.y,d.r,0,Math.PI*2);ctx.fill()});bullets.forEach(b=>{ctx.fillStyle=b.c;ctx.shadowColor=b.c;ctx.shadowBlur=12;ctx.beginPath();ctx.arc(b.x,b.y,b.r,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0});enemies.slice().sort((a,b)=>a.y-b.y).forEach(drawChicken);for(let i=0;i<lv("orbit");i++){const a=frame/28+i*Math.PI*2/lv("orbit");ctx.fillStyle="#ffe66d";ctx.beginPath();ctx.arc(p.x+Math.cos(a)*(48+lv("orbit")*5),p.y+Math.sin(a)*(48+lv("orbit")*5),9,0,Math.PI*2);ctx.fill()}drawPlayer();parts.forEach(o=>{if(o.laser){ctx.strokeStyle="rgba(70,240,255,"+o.life/10+")";ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(o.x1,o.y1);ctx.lineTo(o.x2,o.y2);ctx.stroke()}else{ctx.fillStyle=o.c;ctx.globalAlpha=Math.max(0,o.life/26);ctx.beginPath();ctx.arc(o.x,o.y,3,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1}});texts.forEach(t=>{ctx.fillStyle="rgba(255,255,255,"+t.life/26+")";ctx.font="900 13px Inter, Arial";ctx.fillText(t.v,t.x,t.y)});ctx.restore();ctx.fillStyle="rgba(0,0,0,.28)";ctx.fillRect(14,14,154,8);ctx.fillStyle="#7cffb2";ctx.fillRect(14,14,154*(p.x/WORLD_W),8);ctx.fillStyle="rgba(0,0,0,.28)";ctx.fillRect(14,26,8,96);ctx.fillStyle="#46f0ff";ctx.fillRect(14,26+96*(1-p.y/WORLD_H),8,96*(p.y/WORLD_H));ctx.restore()}
    function loop(){if(state==="play"){frame++;seconds=frame/60;if(frame%(seconds<30?92:seconds<70?74:Math.max(24,64-Math.floor(seconds/10)))===0)wave();update();fire();shots();mobs();pickups();parts.forEach(o=>{o.life--;if(!o.laser){o.x+=o.vx;o.y+=o.vy}});parts=parts.filter(o=>o.life>0);texts.forEach(t=>{t.life--;t.y-=.45});texts=texts.filter(t=>t.life>0);if(frame%60===0)score+=Math.floor(2+seconds/40);hud()}draw();requestAnimationFrame(loop)}
